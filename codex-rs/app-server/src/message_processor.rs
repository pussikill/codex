...
impl ConnectionSessionState {
    ...
    pub(crate) fn mcp_client_capabilities(&self) -> Option<&McpClientCapabilities> {
        self.initialized
            .get()
            .and_then(|session| session.mcp_client_capabilities.as_ref())
    }
...
impl MessageProcessor {
    ...
    async fn dispatch_initialized_client_request(
        ...
        let mcp_client_capabilities = session.mcp_client_capabilities().cloned();
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::PluginRead { params, .. } => {
                self.plugin_processor
                    .plugin_read(params, mcp_client_capabilities.clone())
                    .await
            }
            ...
            ClientRequest::AppsList { params, .. } => {
                self.apps_processor
                    .apps_list(&request_id, params, mcp_client_capabilities.clone())
                    .await
            }
            ...
            ClientRequest::McpServerStatusList { params, .. } => {
                self.mcp_processor
                    .mcp_server_status_list(&request_id, params, mcp_client_capabilities.clone())
                    .await
            }
            ClientRequest::McpResourceRead { params, .. } => {
                self.mcp_processor
                    .mcp_resource_read(&request_id, params, mcp_client_capabilities.clone())
                    .await
            }
